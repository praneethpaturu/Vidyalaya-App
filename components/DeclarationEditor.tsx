    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <form action={updateTaxDeclaration.bind(null, staffId)} className="card card-pad space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <Field name="s80C" label="80C — PF, ELSS, LIC etc. (₹)" defaultValue={(decl?.s80C ?? 0) / 100} hint="Old-regime only · capped ₹1.5L" />
          <Field name="s80D" label="80D — Medical insurance (₹)" defaultValue={(decl?.s80D ?? 0) / 100} hint="Old-regime only · capped ₹25k self / ₹50k senior" />
          <Field name="s80CCD1B" label="80CCD(1B) — NPS extra (₹)" defaultValue={(decl?.s80CCD1B ?? 0) / 100} hint="Capped ₹50k" />
          <Field name="homeLoanInterest" label="Home loan interest (₹)" defaultValue={(decl?.homeLoanInterest ?? 0) / 100} hint="s24(b) · capped ₹2L" />
          <Field name="hraRentPaid" label="Annual rent paid (₹)" defaultValue={(decl?.hraRentPaid ?? 0) / 100} hint="For HRA exemption (old regime)" />
          <Field name="otherIncome" label="Other income (₹)" defaultValue={(decl?.otherIncome ?? 0) / 100} hint="Bank interest, etc." />
        </div>
      ...
      {result && (
        <div className="card mt-4">
          ...
          <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
            ...
            {result.chapter6A > 0 && <Row k="Chapter VI-A (80C/D/CCD1B)" v={`− ${inr(result.chapter6A)}`} />}
          ...
          <div className="px-4 pb-4">
            ...
            <table className="table">
              ...
              <tbody>
                ...
                {result.rebate87A > 0 && <tr><td colSpan={2} className="text-right text-emerald-700">87A rebate</td><td className="text-right text-emerald-700">− {inr(result.rebate87A)}</td></tr>}
                {result.surcharge > 0 && <tr><td colSpan={2} className="text-right">Surcharge</td><td className="text-right">{inr(result.surcharge)}</td></tr>}
