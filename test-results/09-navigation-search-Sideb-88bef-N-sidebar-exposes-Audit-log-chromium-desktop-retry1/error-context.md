...
- Name: 09-navigation-search.spec.ts >> Sidebar — every link reachable for ADMIN >> TC-900.Audit log ADMIN sidebar exposes Audit log
...
Locator: getByRole('link', { name: /Audit log/ }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('link', { name: /Audit log/ }).first()
